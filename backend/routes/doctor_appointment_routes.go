...
func SetupDoctorAppointmentRoutes(api fiber.Router) {
	...
	api.Post("/appointments", func(c *fiber.Ctx) error {
		...
		if docID == 0 {
			return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{"error": "unauthorized: missing doctor context"})
		}
	...
	api.Get("/appointments", func(c *fiber.Ctx) error {
		...
		// ถ้าทั้งคู่ถูกส่งมาและ date_from > date_to ให้สลับให้เลย
		if dfPtr != nil && dtPtr != nil && dfPtr.After(*dtPtr) {
			df, dt := *dtPtr, *dfPtr
			dfPtr, dtPtr = &df, &dt
		}
