...
func SetupNotiInfosRoutes(api fiber.Router) {
	...
	api.Get("/noti-infos", func(c *fiber.Ctx) error {
		...
		if v := c.Query("my_medicine_id"); v != "" {
			if id, err := strconv.ParseUint(v, 10, 64); err == nil {
				filter["my_medicine_id"] = uint(id)
			}
		}
		if v := c.Query("group_id"); v != "" {
			if id, err := strconv.ParseUint(v, 10, 64); err == nil {
				filter["group_id"] = uint(id)
			}
		}
		if v := c.Query("format_id"); v != "" {
			if id, err := strconv.ParseUint(v, 10, 64); err == nil {
				filter["noti_format_id"] = uint(id)
			}
		}
	...
	api.Get("/noti-infos/:type/:id", func(c *fiber.Ctx) error {
		patientID, ok := c.Locals("patient_id").(uint)
		if !ok || patientID == 0 {
		...
		typ := c.Params("type") // "group" หรือ "medicine"
		...
		item, err := handlers.GetNotiInfoByType(db.DB, patientID, typ, uint(id))
		if err != nil {
			status := fiber.StatusInternalServerError
